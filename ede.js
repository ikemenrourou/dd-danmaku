...
(async function () {
  ...
  const dandanplayApi = {
    ...
    getSearchEpisodes: (anime, episode) =>
      `${dandanplayApi.prefix}/search/episodes?anime=${anime}${episode ? `&episode=${episode}` : ''}`,
  ...
  const hasToastPrefixes = (comment, prefixes) =>
  ...
  const classes = {
    ...
    embyTabsDiv1:
      'tabs-viewmenubar tabs-viewmenubar-backgroundcontainer focusable scrollX hiddenScrollX smoothScrollX scrollFrameX emby-tabs',
  ...
  const styles = {
    ...
    rightLayout:
      'position: fixed; right: 0; width: 40%; min-width: auto; min-height: auto; max-width: 100%; max-height: 100%;',
    colors: {
      info: 0xffffff, // 白色
      success: 0x00ff00, // 绿色
      warn: 0xffff00, // 黄色
  ...
  if (!skipInnerModule) {
    ...
    window.Danmaku ||
      Emby.importModule(requireDanmakuPath)
        .then(f => {
          console.log(f);
          window.Danmaku = f;
        })
  ...
  function onPlaybackStopPct(e, state) {
    ...
    if (
      ...
      putBangumiEpStatus(bangumiToken)
        .then(res => {
          embyToast({
            text: `putBangumiEpStatus 成功, 目标: ${targetName}, 结束播放百分比: ${pct}%, 大于需提交的设定百分比: ${bangumiPostPercent}%`,
          });
  ...
  async function lsSeasonSearchEpisodes(_season_key, episode) {
    ...
    if (selectedSeasonInfo) {
      ...
      console.log(
        `命中seasonInfo缓存: ${selectedSeasonInfo.name},偏移量: ${selectedSeasonInfo.episodeOffset},集: ${newEpisode}`,
      );
  ...
  function toastByDanmaku(text, type) {
    ...
    const fontSize = parseFloat(getComputedStyle(getByClass(classes.videoOsdTitle)).fontSize.replace('px', '')) * 1.5;
  ...
  async function showEpisodeDialog() {
    try {
      // 检查是否已经显示，如果是则关闭
      const existingDialog = document.getElementById(eleIds.episodeDialog);
      if (existingDialog) {
      ...
      // 创建弹窗HTML
      const html = `
        <div id="${eleIds.episodeDialog}" style="
          position: fixed;
          bottom: 10vh;
          left: 10vw;
          width: 80vw;
        ...
        ">
          <div id="episode-loading" style="
            ...
            align-items: center;
          ">
            <div style="color: #fff; font-size: 0.9em; opacity: 0.9;">
              正在加载章节页面...
            </div>
            <div style="display: flex; gap: 0.5em;">
              <button onclick="window.open('${episodeUrl}', '_blank')"
                      style="
                        padding: 0.4em 0.8em;
                      ...
                      "
                      onmouseover="this.style.background='rgba(0, 164, 220, 1)'"
                      onmouseout="this.style.background='rgba(0, 164, 220, 0.8)'">
                在新窗口打开
              </button>
              <button onclick="document.getElementById('${eleIds.episodeDialog}').remove()"
                      style="
                        padding: 0.4em 0.8em;
          ...
          <iframe id="${eleIds.episodeIframe}"
                  ...
                  onload="
                    ...
                    if (loadingDiv) {
                      loadingDiv.querySelector('div').textContent = '章节页面加载完成';
                      setTimeout(() => {
                        const textDiv = loadingDiv.querySelector('div');
                        if (textDiv) textDiv.style.opacity = '0.6';
                      }, 2000);
                    }
                  ">
          </iframe>
      ...
      // 直接添加到页面而不是使用embyDialog
      document.body.insertAdjacentHTML('beforeend', html);
    } catch (error) {
      console.error('显示章节信息失败:', error);
      embyAlert({ text: '显示章节信息失败: ' + error.message });
    }
  }

  async function showCharactersInfo() {
    try {
      // 检查是否已经显示，如果是则关闭
      const existingDialog = document.getElementById('charactersDialog');
      if (existingDialog) {
        existingDialog.remove();
        return;
      }
      ...
      // 获取角色信息
      const characters = await getCurrentCharacters();
      ...
      const charactersHtml = characters
        .map(character => {
          const actorName = character.actors && character.actors.length > 0 ? character.actors[0].name : '未知声优';
          const characterImage = character.images ? character.images.medium : '';
          ...
          <div style="
            ...
            backdrop-filter: blur(10px);
          "
          onclick="window.open('https://bgm.tv/character/${character.id}', '_blank')"
          onmouseover="this.style.background='rgba(0, 0, 0, 0.8)'; this.style.transform='translateY(-3px)'; this.style.borderColor='rgba(255, 255, 255, 0.4)'"
          onmouseout="this.style.background='rgba(0, 0, 0, 0.6)'; this.style.transform='translateY(0)'; this.style.borderColor='rgba(255, 255, 255, 0.2)'"
          title="点击查看角色详细信息: ${character.name}">
            ...
            ">
              <img src="${characterImage}"
                   ...
                   "
                   onerror="this.parentElement.innerHTML='<div style=\\'display:flex;align-items:center;justify-content:center;height:100%;background:rgba(255,255,255,0.1);color:#ccc;font-size:0.8em;text-align:center;\\'>暂无图片</div>'"
                   alt="${character.name}">
            </div>
            ...
            ">
              ...
              <div style="
                ...
                text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);
              ">
                <div style="margin-bottom: 0.2em; color: #aaa; font-size: 0.7em;">声优</div>
                ${actorName}
              </div>
            </div>
          </div>
      ...
      const html = `
        ...
        ">
          ...
          ">
            ...
            ">
              角色介绍 (${characters.length}个角色)
            </div>
            <button onclick="document.getElementById('charactersDialog').remove()"
                    style="
                      padding: 0.5em 1em;
                      ...
                      text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);
                    "
                    onmouseover="this.style.background='rgba(0, 0, 0, 0.9)'; this.style.borderColor='rgba(255, 255, 255, 0.5)'"
                    onmouseout="this.style.background='rgba(0, 0, 0, 0.7)'; this.style.borderColor='rgba(255, 255, 255, 0.3)'">
              关闭
            </button>
          </div>
          <div style="
      ...
      // 移除已存在的角色弹窗（这里不需要再检查，因为函数开头已经检查过了）
      document.body.insertAdjacentHTML('beforeend', html);
    } catch (error) {
  ...
  window.showCharactersInfo = showCharactersInfo;
  ...
  function resetFontFamilyDiv(selectedIndexOrValue, opts) {
    ...
    fontFamilyDiv.append(
      embyInput(
        { id: eleIds.fontFamilyInput, value: lsGetItem(lsKeys.fontFamily.id), type: 'search', style: 'display: none;' },
        e => {
  ...
  function buildSearchEpisodeEle() {
    ...
    searchNameDiv.append(
      embyInput(
        { id: eleIds.danmakuSearchName, value: window.ede.searchDanmakuOpts.animeName, type: 'search' },
  ...
  function buildDanmuPluginDiv() {
    getById(eleIds.danmuPluginDiv).append(
      embyCheckbox({ label: lsKeys.useFetchPluginXml.name }, lsGetItem(lsKeys.useFetchPluginXml.id), checked => {
  ...
  function buildCurrentDanmakuInfo(containerId) {
    ...
    let template = `
          ...
          <div style="margin-top: 2%;">
              ...
              <textarea id="${
                eleIds.danmuListText
              }" readOnly style="display: none;resize: vertical;width: 100%" rows="8"
                  is="emby-textarea" class="txtOverview emby-textarea"></textarea>
              <div class="${
                classes.embyFieldDesc
              }">列表展示格式为: [序号][分:秒] : 弹幕正文 [来源平台][用户ID][弹幕CID][模式]</div>
          </div>
    ...
    if (animeId) {
      getById(eleIds.posterImgDiv, container).append(
        embyImgButton(
          embyImg(dandanplayApi.posterImg(animeId)),
  ...
  function buildExtInfo(container) {
    getById(eleIds.characterImgHeihtDiv, container).append(
      embySlider({ labelId: eleIds.characterImgHeihtLabel, value: '12', min: 12, max: 100, step: 1 }, (val, opts) => {
        if (val === '12') {
          val = 'auto';
        }
        onSliderChangeLabel(val, opts);
        Array.from(getById(eleIds.charactersDiv).children).map(
          c => (c.style.height = val === 'auto' ? val : val + 'em'),
        );
      }),
    );
          ...
          <div style="height: 30em;">
              ...
              <div is="emby-collapse" title="弹幕高级屏蔽">
                  <div class="${classes.collapseContentNav}">
                      <div>
                          <div style="${styles.embySlider}">
                              <label class="${classes.embyLabel}" style="width: 10em;">${
      lsKeys.autoFilterCount.name
    }: </label>
                              <div id="${
                                eleIds.danmakuAutoFilterCountDiv
                              }" style="width: 15.5em; text-align: center;"></div>
                              <label style="${styles.embySliderLabel}">0</label>
                          </div>
                          <label class="${classes.embyLabel}">${lsKeys.mergeSimilarEnable.name}: </label>
                          <div id="${eleIds.danmakuFilterProDiv}" class="${classes.embyCheckboxList}" style="${
      styles.embyCheckboxList
    }"></div>
                          <div style="${styles.embySlider}">
                              <label class="${classes.embyLabel}" style="width: 10em;">${
      lsKeys.mergeSimilarPercent.name
    }: </label>
                              <div id="${
                                eleIds.mergeSimilarPercentDiv
                              }" style="width: 15.5em; text-align: center;"></div>
                              <label>
                                  <label style="${styles.embySliderLabel}"></label>
                                  <label>%</label>
      ...
      lsKeys.mergeSimilarTime.name
              ...
              <div is="emby-collapse" title="额外设置">
                  <div class="${classes.collapseContentNav}" style="padding-top: 0.5em !important;">
                      <div id="${eleIds.extCheckboxDiv}" class="${classes.embyCheckboxList}" style="${
      styles.embyCheckboxList
    }"></div>
              ...
              <div is="emby-collapse" title="播放界面设置">
                  <div class="${classes.collapseContentNav}">
                      <div id="${eleIds.osdCheckboxDiv}" class="${classes.embyCheckboxList}" style="${
      styles.embyCheckboxList
    }"></div>
                      <div>
                          <div id="${eleIds.osdLineChartDiv}" class="${classes.embyCheckboxList}" style="${
      styles.embyCheckboxList
    }"></div>
                          <div style="${styles.embySlider}">
                              <label class="${classes.embyLabel}" style="width: 12em;">${
      lsKeys.osdLineChartTime.name
    }: </label>
              ...
              <div is="emby-collapse" title="播放设置">
                  <div class="${classes.collapseContentNav}">
                      ...
                      <div style="${styles.embySlider + 'margin-top: 0.3em;'}">
                          <label class="${classes.embyLabel}" style="width:4em;">${
      lsKeys.timeoutCallbackValue.name
    }: </label>
              ...
              <div is="emby-collapse" title="Bangumi 设置">
                  <div class="${classes.collapseContentNav}" style="padding-top: 0.5em !important;">
                      ...
                      <div id="${eleIds.bangumiSettingsDiv}">
                          ...
                          <div style="${styles.embySlider}">
                              <label class="${classes.embyLabel}" style="width:4em;">${
      lsKeys.bangumiPostPercent.name
    }: </label>
                              <div id="${
                                eleIds.bangumiPostPercentDiv
                              }" style="width: 15.5em; text-align: center;"></div>
                              <label>
                                  <label style="${styles.embySliderLabel}"></label>
                                  <label>%</label>
  ...
  function buildDanmakuFilterSetting(container) {
    ...
    getById(eleIds.danmakuFilterProDiv, container).append(
      embyCheckbox({ label: labels.enable }, lsGetItem(lsKeys.mergeSimilarEnable.id), checked => {
    ...
    keywordsEnableDiv.append(
      embyCheckbox(
        ...
        flag => updateFilterKeywordsBtn(keywordsBtn, flag, getById(eleIds.filterKeywordsId).value.trim()),
      ),
    );
      ...
      embyTextarea(
        ...
        event =>
          updateFilterKeywordsBtn(
            keywordsBtn,
            getById(eleIds.filterKeywordsEnableId).checked,
  ...
  function buildOsdSetting() {
    getById(eleIds.osdCheckboxDiv).append(
      embyCheckbox({ label: lsKeys.osdTitleEnable.name }, lsGetItem(lsKeys.osdTitleEnable.id), checked => {
    ...
    getById(eleIds.osdCheckboxDiv).append(
      embyCheckbox({ label: lsKeys.osdHeaderClockEnable.name }, lsGetItem(lsKeys.osdHeaderClockEnable.id), checked => {
        lsSetItem(lsKeys.osdHeaderClockEnable.id, checked);
        checked ? addHeaderClock() : removeHeaderClock();
      }),
    );
    getById(eleIds.osdLineChartDiv).append(
      embyCheckbox({ label: lsKeys.osdLineChartEnable.name }, lsGetItem(lsKeys.osdLineChartEnable.id), checked => {
  ...
  function buildBangumiSetting(container) {
    ...
    bangumiEnableLabel.append(
      embyCheckbox({ label: lsKeys.bangumiEnable.name }, bangumiEnable, checked => {
    ...
    bangumiTokenInputDiv.append(
      embyInput(
        { id: eleIds.bangumiTokenInput, type: 'password', value: lsGetItem(lsKeys.bangumiToken.id) },
        onEnterBangumiToken,
      ),
    );
    bangumiTokenInputDiv.append(embyButton({ label: '校验', iconKey: iconKeys.check }, onEnterBangumiToken));
    getById(eleIds.bangumiPostPercentDiv, container).append(
      embySlider(
  ...
  async function getCurrentEpisodeUrl() {
    try {
      ...
      // 获取bangumi信息
      const bangumiInfo = await getEpisodeBangumiRel();
      const { subjectId, episodeIndex, bgmEpisodeIndex } = bangumiInfo;

      // 获取章节列表
      const episodesResponse = await fetch(`https://api.bgm.tv/v0/episodes?subject_id=${subjectId}&type=0`, {
        headers: {
          'User-Agent': navigator.userAgent,
        },
      });
      ...
      const targetIndex = bgmEpisodeIndex !== undefined ? bgmEpisodeIndex : episodeIndex;
      const episode = episodes.data[targetIndex];

      if (!episode) {
        throw new Error('未找到对应章节');
      }

      return `https://bgm.tv/ep/${episode.id}`;
    } catch (error) {
  ...
  async function getCurrentCharacters() {
    try {
      ...
      // 获取bangumi信息
      const bangumiInfo = await getEpisodeBangumiRel();
      const { subjectId } = bangumiInfo;

      // 获取角色信息
      const charactersResponse = await fetch(bangumiApi.getCharacters(subjectId), {
        headers: {
          'User-Agent': navigator.userAgent,
        },
      });
  ...
  function buildConsoleLog(container) {
    ...
    consoleLogCtrlEle.append(
      embyCheckbox({ label: lsKeys.consoleLogEnable.name }, consoleLogEnable, doConsoleLogChange),
    );
  ...
  function buildDebugCheckbox(container) {
    ...
    debugWrapper.append(
      embyCheckbox({ label: lsKeys.debugReverseDanmu.name }, lsGetItem(lsKeys.debugReverseDanmu.id), checked => {
    ...
    debugWrapper.append(
      embyCheckbox({ label: lsKeys.debugForceDanmuWhite.name }, lsGetItem(lsKeys.debugForceDanmuWhite.id), checked => {
    ...
    debugWrapper.append(
      embyCheckbox({ label: lsKeys.debugDialogWindow.name }, debugDialogWindowChecked, debugDialogWindowOnChange),
    );
    ...
    debugWrapper.append(
      embyCheckbox({ label: lsKeys.debugDialogRight.name }, debugDialogRightChecked, debugDialogRightOnChange),
    );
    ...
    if (lsGetItem(lsKeys.quickDebugOn.id)) {
      ...
      debugWrapper.append(
        embyCheckbox(
          ...
          lsGetItem(lsKeys.debugTabIframeEnable.id),
          checked => {
  ...
  function buildDebugButton(container) {
    ...
    debugWrapper.append(
      embyButton({ label: '清空章节引用缓存', class: classes.embyButtons.submit, style: 'margin: 0.3em;' }, () => {
  ...
  function buildIframe(containerId) {
    ...
    const template = `
          <div>
              <div class="${
                classes.embyFieldDesc
              }">注意内嵌网页不支持控制器输入,且被禁止内嵌(CSP)的网页无法显示,且跨域无法登录</div>
              <div style="${styles.embySlider + 'margin: 0.8em 0;'}">
      ...
      `;
    container.innerHTML = template.trim();
    getById(eleIds.tabIframeHeightDiv, container).append(
      embySlider({ labelId: eleIds.tabIframeHeightLabel, value: '29', min: 28, max: 100, step: 1 }, (val, opts) => {
        if (val === '28') {
          val = 'auto';
        }
        onSliderChangeLabel(val, opts);
    ...
    getById(eleIds.tabIframeSrcInputDiv, container).append(
      embyInput({ type: 'search', value: window.ede.bangumiInfo ? window.ede.bangumiInfo.bangumiUrl : '' }, e => {
  ...
  async function doDanmakuSearchEpisode() {
    ...
    const animeSelect = embySelect(
      { id: eleIds.danmakuAnimeSelect, label: '剧集: ', style: 'width: auto;max-width: 100%;' },
      selectAnimeIdx,
      animes,
      'animeId',
    ...
    const episodeNumSelect = embySelect(
      { id: eleIds.danmakuEpisodeNumSelect, label: '集数: ', style: 'width: auto;max-width: 100%;' },
      window.ede.searchDanmakuOpts.episode,
      animes[selectAnimeIdx].episodes,
      'episodeId',
      'episodeTitle',
    );
  ...
  function doDanmuListOptsChange(value, index) {
    ...
    danmuListEle.value = value
      .onChange(window.ede)
      .map(
        (c, i) =>
          `[${i + 1}][${f.format(new Date(c.time * 1000))}] : ` +
  ...
  function embySlider(opts = {}, onChange, onSliding) {
    ...
    require(['browser'], browser => {
      if (browser.electron && browser.windows) {
        // Emby Theater
  ...
  function getSettingsJson(space = 4) {
    return JSON.stringify(
      Object.fromEntries(objectEntries(lsKeys).map(([key, value]) => [value.id, lsGetItem(value.id)])),
      null,
      space,
    );
